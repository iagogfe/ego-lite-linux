...
- **A frozen page is reported in under a second, on every path — and the report lifts by itself.** A renderer stuck in a long task (an infinite loop in page JavaScript) cannot answer anything, so the host checks the tab is alive before each page command and says so once. Measured on this build, three runs: the client whose *own* tab is frozen learns in **152ms** (`page.snapshot()`), **~306ms** (`page.url()`, `page.title()`, `locator.count()`, `innerText`, `click`, `fill`) and **~348ms** (`getByRole`) — mean 294-295ms, against 49s, 30s and 15s with three different messages before. A tab that freezes *after* a successful read — the common case, since pages hang while you work with them — is reported in **~303ms**; it used to take 15s. Every path gives the same sentence, naming the space and the tab, and it does not name a CDP method: the verdict is about the tab. Do not probe the tab another way: `page.info()` and CSS reads go to the same renderer and hang just as long. **A page that is only slow is not marked** — a `page.evaluate` that legitimately runs for 6s completes normally, and a healthy 10k-link DOM reads in ~2.8s; the host asks twice before condemning a tab, so a renderer merely busy with its own work answers the second check. Zero false marks in three runs under eight concurrent readers. **Other clients keep working**: with three frozen renderers arriving alongside, a neighbour keeps 73-89% of its solo throughput, and 89-97% with one. **A tab that recovers unblocks itself** — the host re-checks on the next attempt and lets it through, including from a brand-new `ego-browser` process, no `closeTab` needed. When a tab really is gone, the fix the message names works: `const [tab] = await browser.listTabs(); await browser.closeTab(tab.targetId)`, then `browser.openOrReuseTab(url)`.
...
- When a snapshot fails with a CDP timeout on a **frozen** tab, the host names the cause and the fix (close the tab, open a fresh one). Do not probe the frozen tab with `page.info()` or a CSS read: they hang on the same renderer.
