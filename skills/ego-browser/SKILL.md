...
- **A frozen page is reported in seconds, once, on every path — and the report lifts by itself.** A renderer stuck in a long task (an infinite loop in page JavaScript) cannot answer anything. Other clients are barely affected: over 20s windows, baseline 114 reads at p50 171ms; with one frozen renderer alongside, 100 reads at p50 178ms. Nobody queues behind the frozen tab. The client whose *own* tab is frozen learns in about **5 seconds** on the first call and **0-4ms** on every call after (measured 5404ms, then 0-4ms; it used to be 49s, 30s, 30s and 15s with three different messages). Every path — `getByRole`, `page.snapshot()`, `page.url()`, `page.title()`, `locator.count()` — gives the same sentence, naming the space and the tab. Do not probe the tab another way: `page.info()` and CSS reads go to the same renderer and hang just as long. **A page that is only slow is not marked**: `page.evaluate` that legitimately runs for 1.2s, 1.6s, 2.5s or 5s completes normally, because agent script time is never evidence about the renderer. **A tab that recovers unblocks itself**: the host re-checks it on the next attempt and lets it through (measured: blocked while wedged, free 4ms after the page finished, including from a brand-new `ego-browser` process — no `closeTab` needed). When a tab really is gone, the fix the message names works: `const [tab] = await browser.listTabs(); await browser.closeTab(tab.targetId)`, then `browser.openOrReuseTab(url)` (measured 3ms to close, 418ms to reopen).
